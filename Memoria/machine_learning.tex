\chapter{Aprendizaje automático y profundo}
	...
	\section{Perceptrón}
		...
		Como se verá durante esta sección, este modelo ayuda a solucionar problemas de clasificación supervisada. Se dispone de una serie de valores de entrada $x_1, x_2, \hdots, x_n$ y se tiene una serie de valores de salida $y_1, y_2, \hdots, y_m$ que representan a qué clase pertenece la entrada ($2^m$ clases posibles). Esto se consigue mediante la ayuda de sus parámetros, que son una serie de pesos $w_1, w_2, \hdots, w_n$ y un sesgo o \textit{bias} $b$. \\
		...
		Combinando ambas expresiones, y tomando que $\text{sgn}(0) = -1$ en vez de $0$ para obtener una salida binaria, se puede resumir en que la salida del perceptrón es equivalente a la siguiente ecuación. 
		
		$$
		y_1(k) = \begin{cases}
			-1 & \text{si } b_1(k) + \displaystyle\sum_{i=1}^n x_i(k)w_i(k) \leq 0\\
			\hfil1 & \text{si } b_1(k) + \displaystyle\sum_{i=1}^n x_i(k)w_i(k) > 0
		\end{cases}
		$$
		
		Para dar un ejemplo de cómo funciona el perceptrón, se pueden tomar una serie de observaciones que tengan dos valores de entrada y uno de salida, suponiendo que existen dos clases. Esto a fin de cuentas es asignar un valor de $-1$ o 1 a cada punto de $\mathbb{R}^2$ tal y como se describe en la \Cref{fig:labeled_data}. \\
		...
		Una solución rápida sería trazar una recta $r: ax + by + c = 0$ que separe $\mathbb{R}^2$ en dos regiones, de forma que todo punto que pertenezca a una región, pertenezca entonces a una misma clase, tal y como se observa en la \Cref{fig:separated_label_data}. Esta recta suele llamarse \textit{decision boundary} o frontera de decisión. El problema entonces es hallar la recta $r$ de la forma $w_1 x + w_2 y + b = 0$, teniendo que encontrar los parámetros adecuados del modelo. La idea puede extrapolarse a diferente tamaño de entrada tomando un hiperplano de la forma $\textbf{w}^T \textbf{x} + b = 0$ \cite{nndesign}. \\
		...
		\begin{figure}
			...
			\begin{tikzpicture}
				\begin{axis}[ymin = -2.5, ymax = 2.5, xmax = 2.5, xmin = -2.5, xticklabel = \empty, yticklabel = \empty, minor tick num = 1, axis lines = middle, xlabel = $x$, ylabel = $y$, axis on top]
					\addplot[only marks, mark = *] coordinates {(1, 2)};
					\addplot[only marks, mark = triangle*] coordinates {(-1, 2) (0, -1)};
					\addplot[domain = -3:3, samples = 2, dashed] {-2*x+1};
					\draw[fill = gray!30, draw = none] (axis cs:-2.5, -2.5) -- (axis cs:-2.5, 2.5) -- (axis cs: -0.75, 2.5) -- (axis cs:1.75, -2.5);
				\end{axis}
			\end{tikzpicture}
		...
		En cuanto a la pregunta de cómo hallar los parámetros, se considera el \Cref{algo:perceptron} para obtener los parámetros óptimos, donde $\textbf{w}$ es el vector de pesos, $t$ el valor esperado, y $a$ la salida del perceptrón. En dicho algoritmo se supondrá que existe una matriz $X$ de $n$ filas que contiene los diferentes $\textbf{x}$. El algoritmo se basa en la \Cref{eq:perceptron}, que difiere de la propuesta en \cite{nndesign} (en el fondo son lo mismo), pero este cambio facilitará el trabajo más adelante. 
		
		\begin{equation}
			\begin{gathered}
				n(k) = \textbf{w}^T(k)\textbf{x}(k) + b(k)\\\\
				a(k) = \text{sgn}(n(k))\\\\
				\textbf{w}(k+1) = \begin{cases}
					\textbf{w}(k) + t(k)\textbf{x}(k) & \text{si } t(k)a(k) < 0\\
					\hfil \textbf{w}(k) & \text{si } t(k)a(k) > 0
				\end{cases}\\\\
				b(k+1) = \begin{cases}
					b(k) + t(k) & \text{si } t(k)a(k) < 0\\
					\hfil b(k) & \text{si } t(k)a(k) > 0
				\end{cases}
			\end{gathered}
			\label{eq:perceptron}
		\end{equation}
		...
		\begin{algorithm}[!h]
			...
			$b \gets 0$\\
			$k \gets 0$\\
			$\textbf{w} \gets \begin{pmatrix}0\\0\\\vdots\\0\end{pmatrix}$\\
			\Repetir{$e \neq 0$}{
				$e \gets 0$\\
				\Para{$i \gets k$ \KwTo $k + n - 1$}{
					$a(i) \gets \text{sgn}(\textbf{w}^T(i)\textbf{x}(i\pmod{n}) + b(i))$\\
					\eSSi{$t(i)a(i) \leq 0$}{
						$\textbf{w}(i+1) \gets \textbf{w}(i) + t(i)\textbf{x}(i)$\\
						$b(i+1) \gets b(i) + t(i)$\\
						$e \gets e + 1$
					}{
						$\textbf{w}(i+1) \gets \textbf{w}(i)$\\
						$b(i+1) \gets b(i)$\\
					}
				}
				$k \gets k + n - 1$
		...
		La última cuestión que queda por tratar respecto al perceptrón es el porqué se verifica que en el caso de que los puntos dados sean linealmente separables, en un número finito de pasos, el \Cref{algo:perceptron} terminará su ejecución y con el $\textbf{w}$ óptimo, tal y como enuncia el teorema de convergencia del perceptrón. A continuación se presenta su demostración basándose en la que se encuentra en \cite{perceptron_proof}, de forma más clara y simple que la mostrada en \cite{nndesign} al tratar de utilizar las clases $-1$ y 1, en vez de 0 y 1. Para comenzar con la demostración será necesario definir una serie de elementos, comenzando por $\Omega(k)$ y $\textbf{z}(k)$.  
		...
		Con estos elementos es fácil ver que $n(k) = \Omega^T(k)\textbf{z}(k)$. Recordando que en función del signo de $t(k)a(k)$ se deseaba actualizar o no los parámetros de acuerdo a la \Cref{eq:perceptron}, se tiene que $\Omega(k + 1) = \Omega(k) + t(k)\textbf{z}(k)$ en los casos a actualizar. Ahora se considerará un vector $\Omega^*$ de forma que 
		$$
		\exists \Omega^* \forall k  \,\, \text{sgn}\left(\Omega^{*^T}\textbf{z}(k)\right) = t(k),
		$$
		...
		$$
		\delta = \min_k\left\lbrace\left|\Omega^{*^T}\textbf{z}(k)\right|\right\rbrace, 
		$$
		es decir, la distancia al punto más cercano a la frontera de decisión óptima. Con estos elementos se puede comenzar la demostración. La idea es observar cómo $\Omega$ se va pareciendo a $\Omega^*$ tras las iteraciones del algoritmo. Una forma de medir cómo de alineados están dos vectores es el producto escalar. Si $\Omega\cdot\Omega^*$ aumenta tras las iteraciones, es posible que ambos vectores se estén alineando, pero es posible que este valor también aumente si $\Omega(k + 1) = \alpha\Omega(k)$ para algún $\alpha\in\mathbb{R}$, es decir, que se esté reescalando y no alineando. Este segundo caso se puede descartar estudiando cómo cambia el propio vector de pesos $\Omega$. Para realizar la demostración, se analizan las expresiones de $\Omega^T(k+1)\Omega^*$ y $\Omega^T(k+1)\Omega(k+1)$. \\
		
		Con el primero de los términos, al tratar de corregir un error se verifica que
		$$
		\Omega^T(k+1)\Omega^* = (\Omega(k) + t(k)\textbf{z}(k))^T \Omega^* = \Omega^T(k)\Omega^* + \underbrace{t(k)\Omega^{*^T}\textbf{z}(k)}_{\geq\delta}, 
		$$
		donde necesariamente $t(k)\Omega^{*^T}\textbf{z}(k) > 0$, por la definición de $\Omega^{*^T}$. Además, por la manera en la que se ha definido $\delta$, el segundo sumando tiene que pertenecer al intervalo $[\delta, \infty)$, por lo que en una actualización el término aumenta en por lo menos en $\delta$ unidades.  
		...
		$$
		\begin{aligned}
			\Omega^T(k+1)\Omega(k+1) &= (\Omega(k) + t(k)\textbf{z}(k))^T(\Omega(k) + t(k)\textbf{z}(k))\\
			&= \Omega^2(k) + (t(k)\textbf{z}(k))^2 + 2t(k)\Omega^T(k)\textbf{z}(k)\\
			&= \Omega^T(k)\Omega(k) + t^2(k)\underbrace{\textbf{z}^T(k)\textbf{z}(k)}_{\leq 1} + 2\underbrace{t(k)\Omega^T(k)\textbf{z}(k)}_{\leq 0}
		\end{aligned}
		$$
		
		En el segundo sumando se tiene que siempre será menor o igual que 1, pues por definición $0 \leq \textbf{z}^T(k)\textbf{z}(k) = \|\textbf{z}(k)\|^2 \leq 1$. Además, el tercer sumando siempre será cero o negativo, pues se está en el caso en el que la clasificación era incorrecta. De esta situación se puede deducir la siguiente desigualdad, por lo que en una actualización el término sólo varía en como máximo una unidad.   
		...
		Una vez se ha observado cómo varían estos términos al actualizarlos, se puede observar qué pasaría con ellos al hacer $m$ actualizaciones. Con el resultado obtenido en las \Cref{eq:inf_delta,eq:sup_delta} se pueden deducir las desigualdades $\delta m \leq \Omega^T(m)\Omega^*$ y $\Omega^T(m)\Omega(m) \leq m$, que combinándolas se consigue acotar la norma del vector de pesos. 
		
		$$
		\begin{aligned}
			\boxed{\delta m} &\leq \Omega^T(m)\Omega^* = \Omega(m)\cdot\Omega^* = \|\Omega(m)\|\|\Omega^*\|\cos(\theta)\\
			&\leq \|\Omega(m)\| = \sqrt{\Omega^T(m)\Omega(m)}\\
			& \leq \boxed{\sqrt{m}} 
		\end{aligned}
		$$
	...
	\section{Redes neuronales artificiales}
		...
		$$
		aunque existen otras muchas, cada una adecuada a cada tipo de problema y modelo, como por ejemplo el error cuadrático medio, error absoluto, error absoluto medio, error logarítmico, error exponencial, entropía cruzada, etc \cite{funcionesError}. En el caso de problemas de clasificación es muy común la entropía cruzada. 
		
		$$
		L(k) = -\sum \textbf{a}^{(M)}(k)\log_2(\textbf{t}(k))
		$$
		...
		\begin{gathered}
			...
			\delta_i^{(M)} = \frac{\partial L}{\partial a_{i}^{(M)}} \frac{\partial a_{i}^{(M)}}{\partial n_{i}^{(M)}} = \frac{\partial L}{\partial a_{i}^{(M)}} \frac{\partial f_{i}^{(M)}}{\partial n_{i}^{(M)}}\\
		...
		Estas ecuaciones pueden reescribirse de manera matricial para aligerar la notación, que combinándolas con las ideas explicadas referentes al \Cref{algo:descenso}, da lugar al \Cref{algo:backprop}, similar al \Cref{algo:perceptron} pero adaptado a una red neuronal. \\
		...
		\begin{algorithm}[!h]
			...
			\Para{$i \gets 1$ \KwTo $\varepsilon$}{
				\ParaCada{$\textbf{x}^{(1)}$}{
					$\displaystyle\delta^{(M)}(k) \gets \frac{\partial L}{\partial \textbf{a}^{(M)}} \frac{\partial \textbf{f}^{(M)}}{\partial \textbf{n}^{(M)}}$\\
					$\displaystyle\delta^{(m)}(k) \gets \frac{\partial \textbf{f}^{(M)}}{\partial \textbf{n}^{(M)}} W^{(m+1)^T}(k)\delta^{(m+1)}(k)$\\
					\Para{$s \gets 1$ \KwTo $M$}{
						$W^{(s)}(k+1) \gets W^{(s)}(k) - \alpha \delta^{(s)}(k)(\textbf{a}^{(s-1)}(k))^T$\\
						$\textbf{b}^{(s)}(k+1) \gets \textbf{b}^{(s)}(k) - \alpha \delta^{(s)}(k)$
					}
				}
			}
		\end{algorithm}
		
		En esta variante del algoritmo denominada \gls{sgd} se actualizan los parámetros por cada observación del \textit{dataset} y se ha decidido elegir como criterio de parada alcanzar un número de épocas, aunque también se suelen tomar otros criterios, como la magnitud del error. Además, esta variante es computacionalmente costosa, pues para cada observación se calcula el gradiente. Otra variante del algoritmo es el batch. Esta no actualiza por cada observación como \gls{sgd}, halla el gradiente del error promedio de todas las observaciones del \textit{dataset}. No es recomendable, pues es muy costosa y si el \textit{dataset} ocupa mucho, puede no caber en memoria. Ambas aproximaciones se pueden combinar para dar lugar a una variante más eficiente que estas dos, denominada mini-batch, que realiza lo mismo que \textit{batch} pero dividiendo el \textit{dataset} en \textit{minibatches} y realizando una actualización de los parámetros por cada uno de ellos. De esta manera no se necesita tener todo el \textit{dataset} en memoria. \\
		
		En la práctica es poco habitual usar SGD, pues es poco razonable mantener fija la tasa de aprendizaje en todas las iteraciones del algoritmo. Por este motivo, han ido apareciendo versiones optimizadas de SGD. Uno de los más comunes es su versión con momento, que modifica el tamaño de los desplazamientos en función de las direcciones de estos, mismo concepto que en física. Otro de ellos es AdaGrad, que trata de adaptar la tasa de aprendizaje a la frecuencia de cambio de las diferentes componentes del gradiente. Finalmente, uno de los más utilizados y que mejores resultados da es Adam, que combina ideas de los dos últimos presentados \cite{descenso}. 
		
		\subsection{Funciones de activación}
		
			Durante el estudio del perceptrón, se muestra cómo se emplea la función signo como función de activación en la neurona. Esto se debe a que se busca una función que independientemente de los valores de entrada que reciba, produzca una salida binaria. En ciertos casos, no se deseará una salida binaria pues el problema no tendría porqué ser de clasificación, como se ha visto con las redes neuronales. Además, como se ha visto en el \Cref{algo:backprop}, será imprescindible poder calcular la derivada de estas funciones. Por estos motivos, se presentan las funciones de activación más conocidas e importantes \cite{funcionesActivacion,softmax}. 
		...
		\section{Redes neuronales convolucionales}
			...
			\begin{figure}[!h]
				\centering
				\includegraphics[scale = .35]{ejemplo_sobel}
			...
			Habiendo mostrado ya la arquitectura de una red convolucional con sus correspondientes parámetros (kernels y sesgos) e hiperparámetros (\textit{padding} y \textit{stride}), la pregunta clásica es, ¿cómo obtener los parámetros adecuados? La respuesta de cómo obtenerlos es simple, siendo algo más complicada de responder esta vez la de cuáles son. Al no dejar de ser una red neuronal, se utiliza Backpropagation (\Cref{algo:backprop}) también. Aplicando la regla de la cadena en su versión matricial, se pueden obtener términos necesarios para el cálculo del gradiente. 
			
			$$
			\begin{aligned}
				\frac{\partial A^{(s)}}{\partial B^{(s)}} = \frac{\partial F^{(s)}}{\partial B^{(s)}} = \begin{pmatrix}
					\displaystyle\frac{\partial F^{(s)}}{\partial B^{(s)}_{11}} & \cdots & \displaystyle\frac{\partial F^{(s)}}{\partial B^{(s)}_{1p}}\\
					\vdots & \ddots & \vdots\\
					\displaystyle\frac{\partial F^{(s)}}{\partial B^{(s)}_{p1}} & \cdots & \displaystyle\frac{\partial F^{(s)}}{\partial B^{(s)}_{pp}}\\
				\end{pmatrix} &= \frac{\partial F^{(s)}}{\partial N^{(s)}}\frac{\partial N^{(s)}}{\partial B^{(s)}}\\ &= \begin{pmatrix}
					\displaystyle\sum_{i=1}^n\sum_{j=1}^n\frac{\partial F^{(s)}}{\partial N^{(s)}_{ij}}\cancelto{1}{\frac{\partial N^{(s)}_{ij}}{\partial B^{(s)}_{11}}} & \cdots & \displaystyle\sum_{i=1}^n\sum_{j=1}^n\frac{\partial F^{(s)}}{\partial N^{(s)}_{ij}}\cancelto{1}{\frac{\partial N^{(s)}_{ij}}{\partial B^{(s)}_{1p}}}\\
					\vdots & \ddots & \vdots\\
					\displaystyle\sum_{i=1}^n\sum_{j=1}^n\frac{\partial F^{(s)}}{\partial N^{(s)}_{ij}}\cancelto{1}{\frac{\partial N^{(s)}_{ij}}{\partial B^{(s)}_{p1}}} & \cdots & \displaystyle\sum_{i=1}^n\sum_{j=1}^n\frac{\partial F^{(s)}}{\partial N^{(s)}_{ij}}\cancelto{1}{\frac{\partial N^{(s)}_{ij}}{\partial B^{(s)}_{pp}}}\\
				\end{pmatrix}
			\end{aligned}
			$$
			$$
			\begin{gathered}
				\frac{\partial A^{(s)}}{\partial K^{(s)}} = \frac{\partial F^{(s)}}{\partial K^{(s)}} = \frac{\partial F^{(s)}}{\partial N^{(s)}}\frac{\partial N^{(s)}}{\partial K^{(s)}} = X^{(s)}\star\frac{\partial F^{(s)}}{\partial N^{(s)}}\\
				\frac{\partial A^{(s)}}{\partial X^{(s)}} = \frac{\partial F^{(s)}}{\partial X^{(s)}} = \frac{\partial F^{(s)}}{\partial N^{(s)}}\frac{\partial N^{(s)}}{\partial X^{(s)}} = \frac{\partial F^{(s)}}{\partial N^{(s)}}\circledast K^{(s)}
			\end{gathered}
			$$
 		
			Se han mostrado las derivadas de la salida de una capa respecto de sus entradas \cite{backpropCNN}, pero de la misma manera que para las redes neuronales clásicas, se necesita calcular derivadas entre diferentes capas, que también se resuelve con ayuda de las sensibilidades. 
			
			$$
			\begin{gathered}
				\Delta^{(M)} = \frac{\partial A^{(M)}}{\partial N^{(M)}} = \frac{\partial F^{(M)}}{\partial N^{(M)}}\\
				\begin{aligned}
					\Delta^{(m)} = \frac{\partial A^{(M)}}{\partial N^{(m)}} &= \frac{\partial A^{(M)}}{\partial N^{(m+1)}}\frac{\partial N^{(m+1)}}{\partial N^{(m)}}\\
					&= \Delta^{(m+1)}\frac{\partial N^{(m+1)}}{\partial N^{(m)}}\\
					&= \Delta^{(m+1)}\frac{\partial N^{(m+1)}}{\partial X^{(m+1)}}\frac{\partial F^{(m)}}{\partial N^{(m)}}
				\end{aligned}
			\end{gathered}
			$$
			
			Como se comentaba anteriormente, a pesar de contar con el algoritmo para hallar los parámetros, no es suficiente para entrenar la red y obtener un resultado adecuado. Las redes convolucionales son un modelo de deep learing que necesita cantidades de datos y un poder computacional muy elevado. Para obtener un resultado decente en un tiempo adecuado se necesitan muchísimos datos de ejemplo y equipos con muchos recursos, como diferentes GPUs y grandes cantidades de memoria. 
		...
		\section{Transformers}
			...
			\subsection{Ejecución y entrenamiento}
				...
				\begin{itemize}
					\item Transformers con codificador y decodificador, normalmente orientados y originalmente diseñados para tareas de traducción. Un ejemplo es T5 de Google \cite{t5}. 
					\item Transformers con sólo codificador, normalmente orientados a tareas de clasificación. Un ejemplo son los \gls{vit} \cite{vit}. 
					\item Transformers con sólo decodificador, normalmente orientados a tareas de generación de contenido. Un ejemplo es GPT de OpenAI \cite{gpt}. 
				\end{itemize}
		...
		\section{Transformers multimodales}\label{sec:transformers_multimodales}
			...
			Gracias a esta idea de tener conceptos en diferentes modalidades representados en un mismo espacio, se pueden realizar tareas muy útiles como la búsqueda de una imagen dada una descripción textual, la búsqueda de la imagen más similar a otra, la búsqueda del texto más similar a otro, etc. Todos estos problemas son en realidad el mismo, pues todos los elementos quedan representados como un vector y basta con comparar los cosenos entre cada par de embeddings, y elegir el mayor de todos, eligiendo así el embedding que tiene una dirección más similar a la dada. En la \Cref{fig:textos_imagenes} se visualiza cómo mediante esta idea es posible asignar a cada una de las $n$ imágenes, un conjunto de $k$ descripciones. 
			
			\begin{figure}[!h]
				\centering
				\includegraphics[scale = .6]{imagen-texto}
				\caption{Asignación de textos a imágenes \cite{clip}}
				\label{fig:textos_imagenes}
			\end{figure}
			
			Existen diversas arquitecturas de transformers multimodales capaces de trabajar con imágenes y texto, aunque la forma de lograr codificar en embeddings conceptos similares representados de manera diferente, no deja de ser trabajo de los algoritmos de atención múltiple y múltiple-cruzada (\Cref{algo:attention_multiple,algo:attention_multiple-cruzada}). En este caso se suele hablar de atención multimodal, que presenta multitud de variantes (\Cref{fig:variantes_crossed}), aunque el objetivo en todos los casos es similar, mezclar los embeddings intramodales para crear embeddings intermodales \cite{multimodal_transformers,atencion_multimodal}. En la \Cref{fig:atencion_multimodal} se observa cómo mediante el algoritmo de atención multimodal es posible segmentar los diferentes objetos de una imagen en base a sus descripciones textuales. 
			
			\begin{figure}[!h]
				\centering
				\includegraphics[width = \textwidth]{variantes_crossed}
				\caption{Variantes del algoritmo de atención multimodal \cite{multimodal_transformers}}
				\label{fig:variantes_crossed}
			\end{figure}
			
			\begin{figure}[!h]
				\centering
				\includegraphics[scale = .115]{atencion_multimodal}
				\caption{Segmentación de objetos en una imagen mediante una descripción textual \cite{atencion_multimodal}}
				\label{fig:atencion_multimodal}
			\end{figure}
			
			Tal y como se comentaba en el \Cref{chapter:estado_arte}, en algunos artículos se ha tratado de combinar diferentes transformers clásicos, \gls{vit}, redes neuronales convolucionales, y otros módulos, dando lugar a una arquitectura de transformer multimodal más sofisticada, como la que se presenta en \cite{arquitectura_multimodal}, mostrada en la \Cref{fig:arquitectura_multimodal}.
			
			\begin{figure}[!h]
				\centering
				\includegraphics[width = .8\textwidth]{arquitectura_multimodal}
				\caption{Arquitectura multimodal \cite{arquitectura_multimodal}}
				\label{fig:arquitectura_multimodal}
			\end{figure} 
		...
		\subsection{Clasificación no supervisada}
			...
			$$
			\bigcup_{i = 1}^k S_i = S, \, \forall (i,j) \, i \neq j \,\land\, S_i \cap S_j = \varnothing, \text{ y } S_i \neq \varnothing
			$$
			...
			\begin{figure}[!h]
				\Tree[.$S_3$ $\textbf{x}_1$ [.$S_1$ $\textbf{x}_3$ $\textbf{x}_2$ ] [.$S_2$ $\textbf{x}_4$ $\textbf{x}_5$ ] ]
				\caption{Dendrograma}
				\label{fig:dendro}
			\end{figure}
			
			Para lograr la partición del dataset de $n$ elementos en $k$ clusters (con $k \leq n$), inicialmente cada elemento se encuentra en un cluster distinto y durante las iteraciones del algoritmo se van uniendo los clusters más próximos, determinando la distancia entre estos mediante una función. Existen multitud de ellas, dando lugar a variantes del algoritmo como single, complete, average, Ward, etc. \cite{ahc_variantes}\\
			...
			\begin{figure}[!h]
				\centering
				\includegraphics[scale = .4]{elbow_ejemplo}
			...
			\begin{algorithm}[!h]
				...
				$\textbf{k} \in \mathbb{R}^n$\\
				\Para{$i \gets 1$ \KwTo $n$}{
					$\boldsymbol{\mu} \gets k-\text{means}(S, i)$\\
					$s_{i} \gets \displaystyle\sum_{j = 1}^i\sum_{\textbf{x}\in S_j}\|\textbf{x}-\boldsymbol{\mu}_j\|^2$\\
					$k_{i} \gets i$
				}
				$k' \gets$ kneedle$(\textbf{s}, \textbf{k})$
			\end{algorithm}
			
			
			La cuestión que queda por resolver es cómo calcular el codo. Este punto se  define como el de mayor curvatura, por lo que si se tiene una función $s(k)$ que represente las puntuaciones para cada valor de $k$, bastaría con hallar el $k$ que maximice la función de curvatura 
			$$
			\kappa_s(k) = \frac{\frac{\partial^2 s}{\partial k^2}}{\left(1+\left(\frac{\partial s}{\partial k}\right)^2\right)^{3/2}}, 
			$$
			pero en este caso $\kappa_s: \mathbb{N} \longrightarrow \mathbb{R}$ y no $\kappa_s: \mathbb{R} \longrightarrow \mathbb{R}$. Para solucionar este problema se suele utilizar el llamado algoritmo \textit{kneedle}. Dicho nombre proviene del juego de palabras knee (rodilla) y needle (aguja), que en el título de la publicación original \cite{kneedle}, se compara con buscar una aguja en un pajar. Tal y como muestra el \Cref{algo:kneedle}, la idea es normalizar los valores, y tomar como punto de curvatura máxima aquel $x$ que maximiza $f(x) - x$. Se puede fijar un umbral para evitar máximos locales. 
			
			\begin{figure}[!h]
				\centering
				\includegraphics[width = .9\textwidth]{kneedle}
				\caption{Punto de mayor curvatura mediante Kneedle \cite{kneedle}}
				\label{fig:kneedle}
			\end{figure}
			...
			A modo de resumen, se muestra en la \Cref{fig:esquema_arq} un diagrama que recoge la arquitectura propuesta para la clasificación no supervisada de imágenes. 
			
			
			\begin{figure}[!h]
				\centering
				\begin{tikzcd}
					&                                                              &                                &                                                                               & k-\text{means} \arrow[rd] &                 \\
					\text{Imágenes} \arrow[r] & \text{Transformer multimodal} \arrow[r, "E"] & \text{Elbow} \arrow[r, "s(k)"] & \text{Kneedle} \arrow[ru, "k \text{ óptimo}"] \arrow[rd, "k \text{ óptimo}"'] &                           & \text{Clusters} \\
					&                                                              &                                &                                                                               & \text{CJA} \arrow[ru]     &                
				\end{tikzcd}
				\caption{Arquitectura propuesta para clasificación no supervisada de imágenes}
				\label{fig:esquema_arq}
			\end{figure}