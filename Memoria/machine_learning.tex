		...
		\section{Transformers}
			...
			\subsection{Codificación}
				...
				\begin{algorithm}
					...
					\Datos{$E \in \mathbb{R}^{n \times d_m}, W_Q \in \mathbb{R}^{d_m \times d_m}, W_K \in \mathbb{R}^{d_m \times d_m}, W_V \in \mathbb{R}^{d_m \times d_m}$}
					\Resultado{$E' \in \mathbb{R}^{n \times d_m}$}
					
					$Q \gets E W_Q$\\
					$K \gets E W_K$\\
					$V \gets E W_V$\\
					$A \gets \text{softmax}_j\left(\frac{\text{mask}\left(QK^t\right)}{\sqrt{d_m}}\right)V$\\
					$E' \gets E + A$
				\end{algorithm}
				
				Consiste en ejecutar en paralelo $h$ cabezas de atención, cada una con sus respectivas matrices $W_Q^{(i)}$, $W_K^{(i)}$, y $W_V^{(i)}$, para obtener $h$ matrices $A^{(i)}$. De esta manera, se consigue que cada cabeza aprenda los diferentes contextos que puede adoptar un token, para finalmente entre todas las cabezas, elegir el adecuado. Esto se ve reflejado en el \Cref{algo:attention_multiple}. \\
				...
				\begin{algorithm}
					...
					\Datos{$E \in \mathbb{R}^{n \times d_m}, W_Q^{(i)} \in \mathbb{R}^{d_k \times d_m}, W_K^{(i)} \in \mathbb{R}^{d_k \times d_m}, W_V^{(i)} \in \mathbb{R}^{d_v \times d_m}, W_O \in \mathbb{R}^{h d_v \times d_m}, h$}
					\Resultado{$E' \in \mathbb{R}^{n \times d_m}$}
					$d_k \gets \frac{d_m}{h}$\\
					$d_v \gets d_k$\\
					\Paralelo{
						...
						$A^{(i)} \gets \text{softmax}_j\left(\frac{\text{mask}\left(Q^{(i)}\left(K^{(i)}\right)^{t}\right)}{\sqrt{d}}\right)V^{(i)}$
					}
					$E' \gets E + \left(A^{(1)}\middle|A^{(2)}\middle|\cdots\middle|A^{(h)}\right)W_O$
				\end{algorithm}
				
				El proceso de codificación continúa con una normalización de los embeddings. Para realizar dicha normalización\cite{normalization}, se supone que los elementos de cada embedding siguen una distribución $\mathcal{N}(\mu_i, \sigma_i)$, y se normalizan de acuerdo a la ecuación $E' = \Sigma\Gamma\odot(E - M) + B$, es decir, 
				\begin{align*}
					E' &= \begin{pmatrix}
			...
			\subsection{Decodificación}
				...
				\begin{algorithm}
					...
					\Datos{$E_1 \in \mathbb{R}^{n \times d_m}, E_2 \in \mathbb{R}^{n \times d_m}, W_Q^{(i)} \in \mathbb{R}^{d_k \times d_m}, W_K^{(i)} \in \mathbb{R}^{d_k \times d_m}, W_V^{(i)} \in \mathbb{R}^{d_v \times d_m}, W_O \in \mathbb{R}^{h d_v \times d_m}, h$}
